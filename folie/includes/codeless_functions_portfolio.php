...
function codeless_portfolio_overlay(){
    ...
    <?php if( $portfolio_overlay_icon != 'lightbox_link' ): ?>
        ...
        <a class="entry-link <?php echo esc_attr( $classes ) ?>" <?php echo $target ?> href="<?php echo esc_url($link) ?>" title="<?php esc_attr( the_title() ) ?>"></a>
    <?php endif; ?>
    </div><!-- Entry Overlay -->
...
function codeless_portfolio_overlay_icon_link( $icon, $delay = '400' ){
    ...
    if( $icon == 'lightbox' || $icon == 'link' ): ?>
        <a href="<?php echo esc_url( $link ) ?>" <?php echo $target ?> class="<?php echo esc_attr( $classes ) ?> with_anim" data-delay="<?php echo esc_attr( $delay ) ?>"> 
    <?php endif; ?>
