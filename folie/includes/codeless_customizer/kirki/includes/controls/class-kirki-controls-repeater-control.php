			...
			<script type="text/html" class="customize-control-repeater-content">
				...
				<li class="repeater-row minimized" data-row="{{{ index }}}">
					...
					<div class="repeater-row-content">
						<# _.each( data, function( field, i ) { #>
							...
							<div class="repeater-field repeater-field-{{{ field.type }}}">
								...
								<# } else if ( 'color' === field.type ) { #>
							        ...
							        <label>
							            ...
							            <# if ( field.description ) { #>
							                <span class="description customize-control-description">{{{ field.description }}}</span>
							            <# } #>
							            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'folie' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
								...
								<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
									...
									<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( $l10n['no-image-selected'], 'folie' ); ?>" >
										<# if ( field.default ) { #>
											<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
											<img src="{{{ defaultImageURL }}}">
										<# } else { #>
											<?php esc_attr_e( $l10n['no-image-selected'], 'folie' ); ?>
										<# } #>
									</figure>

									<div class="actions">
										<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( $l10n['remove'], 'folie' ); ?></button>
										<button type="button" class="button upload-button" data-label="<?php esc_attr_e( $l10n['add-image'], 'folie' ); ?>" data-alt-label="<?php esc_attr_e( $l10n['change-image'], 'folie' ); ?>" >
											<# if ( field.default ) { #>
												<?php esc_attr_e( $l10n['change-image'], 'folie' ); ?>
											<# } else { #>
												<?php esc_attr_e( $l10n['add-image'], 'folie' ); ?>
											<# } #>
										</button>
										<# if ( field.default.id ) { #>
								...
								<# } else if ( field.type === 'upload' ) { #>
									...
									<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( $l10n['no-file-selected'], 'folie' ); ?>" >
										<# if ( field.default ) { #>
										...
										<# } else { #>
											<?php esc_attr_e( $l10n['no-file-selected'], 'folie' ); ?>
										<# } #>
									</figure>

									<div class="actions">
										<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( $l10n['remove'], 'folie' ); ?></button>
										<button type="button" class="button upload-button" data-label="<?php esc_attr_e( $l10n['add-file'], 'folie' ); ?>" data-alt-label="<?php esc_attr_e( $l10n['change-file'] , 'folie' ); ?>" >
											<# if ( field.default ) { #>
												<?php esc_attr_e( $l10n['change-file'], 'folie' ); ?>
											<# } else { #>
												<?php esc_attr_e( $l10n['add-file'], 'folie' ); ?>
											<# } #>
										</button>
										<# if ( field.default.id ) { #>
											<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
						...
						<# }); #>
						<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( $l10n['remove'], 'folie' ); ?></button>
					</div>
				</li>
			</script>
			<?php
