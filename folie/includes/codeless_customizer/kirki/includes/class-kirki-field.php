...
if ( ! class_exists( 'Kirki_Field' ) ) {
	...
	class Kirki_Field {
		...
		/* Modified Codeless */
		protected $inline_control = false;

		/* Modified Codeless */
		protected $use_on_simple = false;
		...
		protected function set_kirki_config() {
			...
			$this->kirki_config =  $this->kirki_config;
		...
		protected function set_settings() {
			...
			foreach ( $this->settings as $setting_key => $setting_value ) {
				$settings[ sanitize_key( $setting_key ) ] =  $setting_value;
				// If we're using serialized options then we need to spice this up.
				if ( 'option' === $this->option_type && '' !== $this->option_name && ( false === strpos( $setting_key, '[' ) ) ) {
					$settings[ sanitize_key( $setting_key ) ] =  $this->option_name . '[' . $setting_value.']';
				}
			}
		...
		/* Modified Codeless */
		protected function set_inline_control() {
			
			$this->inline_control = (bool) $this->inline_control;

		}

		/* Modified Codeless */
		protected function set_use_on_simple() {
			
			$this->use_on_simple = (bool) $this->use_on_simple;
		...
		protected function set_js_vars() {
			...
			if ( (empty( $this->js_vars ) || count($this->js_vars) != 0 ) && ! empty( $this->output )  ) {
				$i = 0;
				...
				foreach ( $this->output as $output ) {
					...
					if ( is_array( $output['element'] ) ) {
						$output['element'] = implode( ',', $osutput['element'] );
					}
					...
					//Modified Codeless
					if(isset($this->js_vars[$i])){
						foreach($this->js_vars[$i] as $id => $value){
							$output[$id] = $value;
						}
					}
					$i++;
