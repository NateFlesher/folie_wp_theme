	...
	final class Kirki_Fonts_Google {
		...
		public function enqueue() {
			...
			if( is_array( $this->link ) && ! empty( $this->link ) ){
				foreach( $this->link as $f )
					wp_enqueue_style( 'cl-default-font-'.$f, get_template_directory_uri().'/css/codeless-default-'.$f.'.css', array(), null );
			}else if ( '' !== $this->link ) {
		...
		private function create_link() {
			...
			foreach ( $this->fonts as $font => $variants ) {
				...
				if( $font == 'Poppins' || $font == 'Source Sans Pro' ){
					$cl_fonts[] = strtolower(str_replace( ' ', '_', $font ));
					if( count( $this->fonts ) > 2 )
