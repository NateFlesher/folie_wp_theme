...
CL_POSTMESSAGE.logo_font = function(to){
    CL_POSTMESSAGE.generate_font_css( 'logo_font', to );
};


CL_POSTMESSAGE.headings_typo = function(to, setting){
    CL_POSTMESSAGE.generate_font_css( 'headings_typo', to );
};


CL_POSTMESSAGE.body_typo = function(to, setting){
    CL_POSTMESSAGE.generate_font_css( 'body_typo', to );
};


CL_POSTMESSAGE.menu_font = function(to){
    CL_POSTMESSAGE.generate_font_css( 'menu_font', to );
};

CL_POSTMESSAGE.dropdown_hassubmenu_item = function(to){
    CL_POSTMESSAGE.generate_font_css( 'dropdown_hassubmenu_item', to );
};

CL_POSTMESSAGE.dropdown_item_typography = function(to){
    CL_POSTMESSAGE.generate_font_css( 'dropdown_item_typography', to );
};


CL_POSTMESSAGE.widgets_title_typography = function(to){
    CL_POSTMESSAGE.generate_font_css( 'widgets_title_typography', to );
};

CL_POSTMESSAGE.blog_entry_title = function(to){
    CL_POSTMESSAGE.generate_font_css( 'blog_entry_title', to );
};

CL_POSTMESSAGE.single_blog_title = function(to){
    CL_POSTMESSAGE.generate_font_css( 'single_blog_title', to );
};

CL_POSTMESSAGE.portfolio_item_title = function(to){
    CL_POSTMESSAGE.generate_font_css( 'portfolio_item_title', to );
}

CL_POSTMESSAGE.portfolio_item_categories = function(to){
    CL_POSTMESSAGE.generate_font_css( 'portfolio_item_categories', to );
}

CL_POSTMESSAGE.button_font_custom = function(to){
  CL_POSTMESSAGE.generate_font_css('button_font_custom', to);
}

CL_POSTMESSAGE.blog_entry_readmore = function(to){
  CL_POSTMESSAGE.generate_font_css('blog_entry_readmore', to);
}

CL_POSTMESSAGE.header_top_typography = function(to){

  CL_POSTMESSAGE.generate_font_css('header_top_typography', to);
}
  
  
  /*wp.customize( 'logo_font', function( value ) {
      value.bind( function( to ) {
        cl_front_logo_font(to);
      });
  });*/
...
CL_POSTMESSAGE.header_container = function(to){
  if( to == 'container-fluid'){
    $('.header_container .header-row-inner').addClass('container-fluid').removeClass('container');
  }else{
    $('.header_container .header-row-inner').addClass('container').removeClass('container-fluid');
  }
}
...
CL_POSTMESSAGE.header_design_top = function(to, setting){
  ...
  $('.header_container  > .top_nav').css(to);
...
CL_POSTMESSAGE.header_design_extra = function(to, setting){
  ...
  $('.header_container  > .extra_row').css(to);
...
CL_POSTMESSAGE.blog_boxed = function(to, setting){
  if( to )
    $('#blog-entries article.default-style').addClass( 'boxed-layout' );
  else
    $('#blog-entries article.default-style').removeClass( 'boxed-layout' );
}
...
CL_POSTMESSAGE.shop_item_distance = function(to){
  $( '.woocommerce-page .shop-products' ).css({ marginLeft: '-'+to+'px', marginRight: '-'+to+'px' });
  $( '.woocommerce-page .shop-products .product_item .inner-wrapper' ).css('padding', to+'px') ;
}

CL_POSTMESSAGE.shop_animation = function(to, setting){
  var effects = [
    'top-t-bottom',
    'bottom-t-top',
    'right-t-left',
  ...
  effects.forEach(function(toDelete){
    $('.woocommerce-page .shop-products .product_item').removeClass(toDelete);
  });

  
  if( to != 'none' ){
    $('.woocommerce-page .shop-products .product_item').addClass(to);
  }
};

CL_POSTMESSAGE.shop_item_heading = function(to, setting){
  var choices = [
    'h1',
    'h2',
    'h3',
  ...
  choices.forEach(function(toDelete){
    $('.shop-products .product_item h3').removeClass(toDelete);
  });

  $('.shop-products .product_item h3').addClass(to);
}
...
CL_POSTMESSAGE.meta_page_fullwidth_content = function(to){
...
if(!_.isUndefined(parent.wp.customize.settings.settings)){
    var dependency = parent.window['newvars'];
    _.each(parent.wp.customize.settings.settings, function(key, setting){

        if(!_.isUndefined(CL_POSTMESSAGE[setting]) || !_.isUndefined(dependency[setting])){
          wp.customize( setting, function(value){
            value.bind( function(to){
              ...
              if(!_.isUndefined(dependency[setting])){
                
                
                _.each(dependency[setting], function(key, index){
                      ...
                      if(!_.isArray(key['value'])){
                        if(to == key['value']){
                          var current = key['current'];
                            ...
                            var changeFunc = function(control){
                              var value = control.setting.get();
                              control.setting.set(false);
                              control.setting.set(value);
                            };
                            
                            parent.wp.customize.control(current, changeFunc);
                        ...
                        _.each(key['value'], function(key_, index_){
                          if(key_ == to){
                            bool = true;
                            return;
                          }
