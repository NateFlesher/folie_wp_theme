...
class Codeless_Post_Like
{
    ...
    static function post_like($post_id, $action = 'get') {
        ...
        switch ($action) {
            ...
            case 'update':
                $love_count = get_post_meta($post_id, '_codeless_post_like', true);
                if (isset($_COOKIE['codeless_folie_post_like_' . $post_id])) return $love_count;
                
                $love_count++;
                update_post_meta($post_id, '_codeless_post_like', $love_count);
                setcookie('codeless_folie_post_like_' . $post_id, $post_id, time() * 20, '/');
    ...
    static function like($icon = 'cl-icon-heart4') {
        global $post;
        ...
        if (isset($_COOKIE['codeless_folie_post_like_' . $post->ID])) {
            $class = 'item-liked';
        }
